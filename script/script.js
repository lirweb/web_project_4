...
function openPopup(popup) {
    ...
    overlay.addEventListener('click', overlayClickToEscPopup);
    document.addEventListener("click",closePopupWithEsc);
};

//closing popup
function closePopup(popup) {
    popup.classList.remove("popup_on");
    overlay.removeEventListener('click', overlayClickToEscPopup);
    document.removeEventListener("click",closePopupWithEsc);
}
...
function closePopupWithEsc(evt) {
  ...
  */
  ...
  function closePopupWithEsc(evt) {
    if (evt.key === "Escape") {
      const popup = document.querySelector(".popup_on");
...
function overlayClickToEscPopup(evt) {
    closePopup(evt.target);
}
...
document.onkeydown = function(evt) {
    ...
    if(evt.keyCode == 27){
        ...
        //document.getElementById(popupOn).setAttribute("visibility:hidden");
